		...
		$.ajax({
			...
			,success:function(data) {
				...
				if(state=="true") {
					var date="${date}";
					var num="${dto.num}";
					location.href="<%=cp%>/schedule/day.do?date="+date+"&num="+num;
				}
			}
...
function deleteOk(num) {
	if(confirm("일정을 삭제 하시 겠습니까 ? ")) {
		var date="${date}";
		var url="<%=cp%>/schedule/delete.do?date="+date+"&num="+num;
		location.href=url;
	}
}
</c:if>
							    ...
							    <c:if test="${dto.num != vo.num}">
							      <tr align="center" bgcolor="#ffffff" height="35" style="border-bottom: 1px solid #cccccc;">
							           ...
							           <td align="left" style="padding-left: 10px;">
							               <a href="<%=cp%>/schedule/day.do?date=${date}&num=${vo.num}">${vo.subject}</a>
							           </td>
							           <td>${vo.created}</td>
							      </tr>
							     </c:if>
