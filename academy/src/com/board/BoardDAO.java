...
public class BoardDAO {
    ...
    public int dataCount(String condition, String keyword) {
        ...
        try {
        	if(condition.equals("created")) {
        		...
        		sql="SELECT NVL(COUNT(*), 0) FROM board b JOIN member m ON b.userId=m.userId WHERE TO_CHAR(created, 'YYYYMMDD') = ?  ";
        	} else if(condition.equals("userName")) {
        		sql="SELECT NVL(COUNT(*), 0) FROM board b JOIN member m ON b.userId=m.userId WHERE INSTR(userName, ?) = 1 ";
        	} else {
        		sql="SELECT NVL(COUNT(*), 0) FROM board b JOIN member m ON b.userId=m.userId WHERE INSTR(" + condition + ", ?) >= 1 ";
        	}
	...
	public List<BoardDTO> listBoard(int offset, int rows) {
		...
		try {
			sb.append("SELECT boardNum, m.userId, m.userName, ");
	...
	public BoardDTO readBoard(int boardNum) {
		...
		try {
			sb.append("SELECT boardNum, m.userId, m.userName, subject, ");
